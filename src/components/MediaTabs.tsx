...
export const VIDEO_TAGS = ["gameplay", "profissional", "roblox", "vlog", "tutorial", "música"];

export const VideosGrid = ({ selected }: { selected: string[] }) => {
  const items = useMemo(
    () =>
      selected.length === 0
        ? VIDEOS
  ...
  if (items.length === 0) {
    return (
      <div className="text-center digital-text text-sm text-muted-foreground py-6">
...
export const IMAGE_TAGS = ["thumbnail", "montagem", "fanart", "wallpaper"];

export const ImagesGrid = ({ selected }: { selected: string[] }) => {
  const items = useMemo(
    () =>
      selected.length === 0
        ? IMAGES
